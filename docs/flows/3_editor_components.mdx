...
Windmill now supports **PEP-723** inline script metadata for Python scripts, providing a standardized way to specify script dependencies and Python version requirements directly within your script. This implements the official Python packaging standard for inline script metadata.

#### PEP-723 format

You can use the standard PEP-723 format to specify dependencies and Python version requirements:
...
This shortcut is equivalent to specifying `requires-python = ">=3.12"` in the PEP-723 format. You can use version specifiers like:
- `#py: ==3.12.*` - Exact Python 3.12 family
- `#py: >=3.11,<3.14` - Range specification
- `#py: >=3.12` - Minimum version
