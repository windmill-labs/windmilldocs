---
description: What is the architecture of Windmill flows and how do steps exchange data using input transforms?
---

import DocCard from '@site/src/components/DocCard';
import Tabs from '@theme/Tabs';
import TabItem from '@theme/TabItem';
import FlowEngineSimulator from '@site/src/components/FlowEngineSimulator';
...
Each step of a flow is executed as an individual [job](../core_concepts/20_jobs/index.mdx) picked up by a [worker](../core_concepts/9_worker_groups/index.mdx). Sequential steps run one after another, while [parallel branches](./13_flow_branches.md#branch-all) are queued simultaneously and can be processed by different workers at the same time. A join step (like a final result aggregation) waits until every branch has completed before it is queued. With enough workers, parallel branches overlap and the total flow duration equals the longest branch — not the sum of all branches.
...
Flow expressions (input transforms, branch predicates, for-loop iterators) are evaluated using a JavaScript engine. Two engines are available:

- **QuickJS**: A lightweight JavaScript engine with 8-16x faster startup times for simple expressions. It uses lazy evaluation — `resource()` and `variable()` calls are resolved on demand rather than pre-fetched, which is more efficient for expressions that don't use all available resources.
- **Deno (V8)**: The full V8-based JavaScript engine with broader compatibility.

On Community Edition, QuickJS is the default engine. To switch back to Deno, set the `USE_DENO_FOR_FLOW_EVAL` environment variable.

On [Enterprise Edition](/pricing), Deno remains the default. To opt into QuickJS, set the `USE_QUICKJS_FOR_FLOW_EVAL` environment variable.
...
- Static inputs: fixed values set directly in the step input fields.
- [Flow env variables](./3_editor_components.mdx#flow-env-variables): flow-level constants that can be referenced in any step input using `flow_env.VARIABLE_NAME` or `flow_env["VARIABLE_NAME"]`.
- Dynamic inputs:
