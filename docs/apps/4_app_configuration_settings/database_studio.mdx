---
description: What is the Database studio component in Windmill? Browse, query and edit database content with a built-in management tool.
---
...
- Create a new table: Open the table editor to define columns, types, primary keys, default values and foreign keys, then generate and execute the `CREATE TABLE` SQL.
- Alter an existing table: Right-click on a table name and select "Alter table" to modify its schema. You can rename columns, change types, add or remove columns, and update foreign keys. The editor shows a SQL preview of the `ALTER TABLE` statements before execution. A warning is displayed for databases that do not support transactional DDL (changes cannot be rolled back).
...
- `Type`: [PostgreSQL](../../getting_started/0_scripts_quickstart/5_sql_quickstart/index.mdx#postgresql), [MySQL](../../getting_started/0_scripts_quickstart/5_sql_quickstart/index.mdx#mysql), [MS SQL](../../getting_started/0_scripts_quickstart/5_sql_quickstart/index.mdx#ms-sql), [Snowflake](../../getting_started/0_scripts_quickstart/5_sql_quickstart/index.mdx#snowflake), [BigQuery](../../getting_started/0_scripts_quickstart/5_sql_quickstart/index.mdx#bigquery), or [Data Table](../../core_concepts/11_persistent_storage/data_tables.mdx).
- `Resource`: The actual [resource](../../core_concepts/3_resources_and_types/index.mdx) from the above resource type. For Data Tables, a table picker lets you select from the workspace's configured data tables.
