...
- an [app builder](./getting_started/7_apps_quickstart/index.mdx) to build application and data-intensive dashboards built with low-code or JS frameworks such as React.

Windmill can be used solely from its UI through its [webIDE](./code_editor/index.mdx), and low-code builders but it is also possible to keep using your editor and deploy from a [git repo](./advanced/12_deploy_to_prod/index.mdx) using a [CLI](./advanced/3_cli/index.mdx).
