...
Windmill can be used solely from its UI through its [webIDE](./code_editor/index.mdx), and low-code builders but it is also possible to keep using your editor and deploy from a [git repo](./advanced/12_deploy_to_prod/index.mdx) using a [CLI](./cli_local_dev/index.mdx).
