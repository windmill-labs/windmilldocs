...
You can create a user token in the Windmill app. Follow the instructions on the [Webhooks docs](/docs/core_concepts/webhooks#user-token).

For TypeScript scripts, the Windmill extension will use by default [Bun](../../getting_started/0_scripts_quickstart/1_typescript_quickstart/index.mdx#bun) as the runtime. You can change it either per script by using file extension `.deno.ts` or globally in your [wmill.yaml](../../advanced/3_cli/sync.mdx#wmillyaml) in the field `defaultTs`.