...
In the [Community Edition](../7_plans_details/index.mdx), worker management is done using tags that can be respectively assigned to workers (through the [env variable](#how-to-assign-worker-tags-to-a-worker-group) `WORKER_TAGS`) and scripts or flows, so that the workers listen to specific jobs queues.

Under an [Enterprise license](../7_plans_details/index.mdx), worker groups can be managed through Windmill UI.

Specifically, you can group the workers into worker groups (a single worker can be in several groups), groups for which you can [manage the tags](../../core_concepts/9_worker_groups/index.mdx), [assignment to a single script](../../core_concepts/25_dedicated_workers/index.mdx), or the [worker init scripts](../../advanced/8_preinstall_binaries/index.mdx#init-scripts), from the UI.
