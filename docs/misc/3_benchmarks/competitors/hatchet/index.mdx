...
<Tabs>
  ...
  <TabItem value="go" label="Go">
    ...
    func main() {
        n := flag.Int("n", 10, "Fibonacci number to compute or number of bulk iterations")
        ...
        if *bulk {
            ...
            // Create `n` inputs with the same N value
            var inputs []workflows.FibonacciInput
            for i := 0; i < *n; i++ {
                inputs = append(inputs, workflows.FibonacciInput{
                    N: *n,
                })
            }
            ...
            fmt.Printf("Running single Fibonacci workflow for n=%d...\n", *n)
