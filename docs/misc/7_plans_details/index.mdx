...
- As an Enterprise user, you will have access to detailed usage information and invoices through the [Windmill billing portal](https://windmill.dev/billing) where you can update plan usage and adjust the number of seats.
...
- Upon subscribing, we will get back to you with a [license key](#using-the-license-key-self-host) to pass in the Instance Settings. The first key will be valid for a month (duration of trial). You can also contact us immediately if preferred. At the end of the subscription period (or trial), you will receive another license key valid for a year. The key will then self-update as long as they are in good standing.
...
- **What is an operator?**
  ...
  An [operator](../../core_concepts/16_roles_and_permissions/index.mdx) is a user who can only execute scripts, flows and apps, but not create and edit them. This is enforced by both a simplified frontend and a restricted API. Operators are 1/2 price of developers (or 1/2 seats).
