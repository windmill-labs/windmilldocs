...
Alternatively, you can set a global version by configuring the `INSTANCE_PYTHON_VERSION` environment variable to one of the mentioned versions.
If you leave `INSTANCE_PYTHON_VERSION` empty it will inherit "Latest Stable" version, which depends on windmill version and currently is 3.11.
...
:::note
Only one python version can be annotated within a single tree.
:::
