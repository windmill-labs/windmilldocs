...
The service account name `windmill` is generated by the Helm chart's fullname template (matching the release name). Replace `windmill:windmill` with `<namespace>:<service-account-name>` if you use different values.
