...
1. Go to AWS VPC and create a new one.
   ...
   - We recommend using at least 2 availability zones, with 2 public subnets and 2 private subnets
   - NAT is required for ECS containers to have access to internet. We recommend creating 2, one in each AZ.
   - We recommend using the S3 Gateway if you're planning to leverage S3 in Windmill
...
1. Template
   - Choose the template of your choice (Production VS Dev/Test) depending on your needs. Obviously this will greatly impact costs
...
1. Settings
   - Name it `windmill-db`
   - Leave `postgres` as the username, and choose a string master password. Keep it in a safe place, you will need it in the following steps
1. Instance configuration
   - The instance will be preset depending on the template you chose. Default is usually a good starting point
...
1. Connectivity
   ...
   - Attach it the VPC created above
...
1. Infrastructure
   - Uncheck AWS Fargate and check Amazon EC2 instance instead
   - Create a new ASG On-Demand
   ...
   - Choose an EC2 instance type matching the OS chose above. Here we're using the default Amazon Linux 2 OS, so we will choose a `t3.medium`
   - This Auto-scaling Group will host the 2 Windmill servers, the multi-purpose and native Windmill workers, LSP and multiplayer. The maximum capacity should be at least 4 hosts
...
1. Container:
   ...
   - Add the following healthcheck: `CMD-SHELL, curl -f http://localhost:8000/api/version || exit 1` / 10s interval / 5s timeout and 5 retries
