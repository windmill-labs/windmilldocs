...
description: How do I deploy Windmill on AWS using EKS, ECS, or CloudFormation?
---
...
IAM RDS authentication is only available in Windmill [Enterprise Edition](/pricing).
:::
...
```
postgresql://postgres:iamrds@<rds-endpoint>/windmill
```
...
On **EKS**, use either:
- [EKS Pod Identity](https://docs.aws.amazon.com/eks/latest/userguide/pod-identities.html) (recommended for new setups)
- [IRSA](https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html) (IAM Roles for Service Accounts)

On **ECS**, attach an IAM role with the `rds-db:connect` permission to your task definition.
