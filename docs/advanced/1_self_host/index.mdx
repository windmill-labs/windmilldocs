...
- The Windmill container runnning in worker mode (and replicated to handle more job throughput). It needs to connect to the database and does not communicate to the servers.
...
It also redirects TCP traffic on port 25 to Windmill (port 2525) for email triggers. 
...
From your self-hosted instance, click on your username and pick [Instance settings](../18_instance_settings/index.mdx#smtp) and "SMTP" tab.

![SMTP instance settings](../18_instance_settings/smtp.png 'SMTP instance settings')
