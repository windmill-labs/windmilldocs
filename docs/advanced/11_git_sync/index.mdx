...
![Git sync diagram](./git_sync_diagram.png 'Git sync diagram')
...
## Promotion: Cross-instance deployment using a git workflow

Promotion mode is an advanced setup leveraging the same tech as git sync for deploying from a workspace to another, even across different instances. Unlike standard git sync which commits directly to a branch, promotion mode creates separate branches per deployment (prefixed with 'wm_deploy/') to enable pull request workflows between staging and production instances.

This setup does not replace git sync, in fact both workspaces should have git sync setup in the first place for it to work. To better understand, let's walk through an example.

- There are two workspaces `staging` and `prod`, setup with git sync to their respective repositories `staging-repo` and `prod-repo`. `staging` is also set to target `prod-repo` for promotion.
- A user makes a change in the script "f/shared/amazing_script" in workspace "staging".
- This triggers the promotion, which will create a branch named "wm_deploy/f/shared/amazing_script" on the repository `prod-repo` with the changes.
...
![Cross-instance deployment](../4_local_development/local_development.png 'Deploying to prod')

<br />

### Configuring promotion

1- Setup both workspaces with git sync into their respective repositories

Promotion is configured per repository in Git Sync settings. When enabled, you can choose to:
...
You can configure promotion-specific sync behavior using `promotionOverrides` in your [`wmill.yaml`](../3_cli/sync.mdx#wmillyaml) file.
