---
description: How do I manage dependencies and imports across languages in Windmill?
---
...
For [R](../../getting_started/0_scripts_quickstart/15_rlang_quickstart/index.mdx), dependencies are automatically detected from `library()` and `require()` calls. Windmill resolves versions from CRAN and caches lockfiles with a 3-day TTL.
...
The lockfile is not meant to be edited manually. It is generated by Windmill when the script is created or edited through the UI and updated locally with the `wmill generate-metadata` command. However, it is useful to check versions changes.

[Windmill CLI](../3_cli/index.mdx) command `wmill generate-metadata` will read all the files that have been edited and gracefully update the metadata file and lockfile accordingly.
