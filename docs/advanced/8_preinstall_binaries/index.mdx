...
To have packages be available for import, place them in `/tmp/windmill/cache/python_xyz/global-site-packages` where xyz is the python version intended. For python 3.11, the path would be `/tmp/windmill/cache/python_311/global-site-packages`.
