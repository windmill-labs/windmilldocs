...
- `groups`: an array of strings containing the groups to which the user belongs. When using JWT external authentication, **group permissions are not checked** and can be left empty most of the time because the user's permissions depend directly on the folders you have specified. The only place where groups could still be useful is in apps, where the user's groups are passed on as context. 
