...
Windmill's strength lies in its ability to run scripts without having to deal with separate dependency files. This is achieved by automatically parsing the [imports](../6_imports/index.mdx) and resolving the dependencies.
...
When using Bun as the runtime for TypeScript in Windmill, dependencies are included directly in the script, and the TypeScript runtime Bun ensures 100% compatibility with Node.js without requiring any code modifications.
...
The lockfile for a file generated with [wmill CLI](../3_cli/index.mdx) command [`wmill script generate-metadata`](../3_cli/script.md#re-generating-a-script-metadata-file) will be overriden by the closest package.json from a parent or current folder if there is one.
