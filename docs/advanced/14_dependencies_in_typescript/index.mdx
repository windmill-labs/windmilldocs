...
If a token is required for authentication, the registry URL should be modified to include `:_authToken=<token_value>`, where `<token_value>` is the actual authentication token.
