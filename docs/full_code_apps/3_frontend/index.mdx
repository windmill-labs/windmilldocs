...
Full-code apps support [React](https://react.dev/) and [Svelte](https://svelte.dev/) frontends. Your code is bundled by Windmill and served as a single-page application. The auto-generated `wmill.ts` module provides typed functions to call [backend runnables](../2_backend_runnables/index.mdx).
...
The [CLI](../5_cli_workflow/index.mdx) generates a starter project with the selected framework, including `package.json`, entry point and a sample component.
