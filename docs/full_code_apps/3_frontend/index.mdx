---
description: Build frontends with React, Svelte or Vue and call Windmill backend runnables using the wmill.ts module.
---
...
Full-code apps support React, Svelte and Vue frontends. Your code is bundled by Windmill and served as a single-page application. The auto-generated `wmill.ts` module provides typed functions to call [backend runnables](../2_backend_runnables/index.mdx).
...
onMounted(async () => {
  users.value = await backend.get_users({ limit: 50 });
  loading.value = false;
});
</script>
