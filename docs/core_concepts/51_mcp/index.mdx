---
description: How do I connect LLMs to Windmill? Use the Model Context Protocol (MCP) to trigger scripts and flows from Claude, Cursor or any MCP client.
---
...
You can quickly connect your MCP client to Windmill by simply adding the following URL to your client configuration:

```
https://app.windmill.dev/api/mcp/w/<your-workspace-id>/mcp
```

This OAuth-based approach automatically handles authentication and provides access to all available tools in your workspace.
...
```
https://app.windmill.dev/api/mcp/w/<your-workspace-id>/mcp?token=<your-token>
```
...
You can also trigger multi-step [flows](../6_flows/index.mdx) and use the built-in API endpoint tools to monitor them:
