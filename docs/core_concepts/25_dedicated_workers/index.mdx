...
Without workspace dependencies, each dedicated worker script gets its own process. With shared runners, scripts that reference the same workspace dependency (e.g. `# requirements: myfile`) and use the same language are grouped into one subprocess that dispatches jobs to the correct script based on its path. This avoids having many parallel processes and reduces memory usage when you have a large number of dedicated worker scripts sharing the same dependencies.
