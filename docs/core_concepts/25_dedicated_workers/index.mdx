...
When dedicated worker scripts in the same worker group share a [workspace dependency](../55_workspace_dependencies/index.mdx) and language, they automatically run in a single long-lived subprocess instead of separate processes. This is called a shared runner.

Without workspace dependencies, each dedicated worker script gets its own process. With shared runners, scripts that reference the same workspace dependency (e.g. `# requirements: myfile`) and use the same language are grouped into one subprocess that dispatches jobs to the correct script based on its path. This avoids having many parallel processes and reduces memory usage when you have multiple dedicated worker scripts sharing the same dependencies.
...
To have scripts share a runner:
...
2. Reference the same workspace dependency from multiple scripts using annotations (e.g. `# requirements: myfile` for Python or `// package_json: myfile` for TypeScript).
...
The worker group configuration UI displays which scripts share a runner.
...
[Preprocessors](../43_preprocessors/index.mdx) are supported on dedicated workers and bunnative scripts. This means scripts assigned to dedicated workers for lower latency can also use preprocessors for HTTP triggers, webhooks, and other trigger types.
