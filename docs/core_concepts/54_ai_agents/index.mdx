...
Conceptually, tools here are equivalent to tools exposed by an MCP server. The AI agent acts as the MCP client: it selects a tool by name and issues a tool call with JSON arguments that conform to the tool’s input schema; Windmill executes the underlying `script` and returns a JSON result. The result is surfaced back to the model as a tool response message and is included in `messages`.

When tools are configured, the AI agent can decide when and how to use them based on the user's request—invoking tools the same way an MCP client invokes MCP tools—combining text generation with practical actions.
