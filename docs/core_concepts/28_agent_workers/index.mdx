...
In terms of [billing](../../misc/7_plans_details/index.mdx), agent workers are counted as 1 [Compute Unit](/pricing#compute-units) per agent worker on self-hosted EE and 0.5 Compute Unit on cloud EE (therefore same price as on the self-hosted plan).
