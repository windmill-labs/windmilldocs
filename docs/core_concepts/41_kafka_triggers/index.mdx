...
For existing triggers, you can force the consumer to re-read all messages from the beginning of the topic by clicking the **Reset offset to earliest** button in the trigger editor. A confirmation modal will appear since this is a destructive action — the consumer will restart and re-process all messages from the beginning.

After the reset, normal consumption resumes from committed offsets on subsequent restarts.
