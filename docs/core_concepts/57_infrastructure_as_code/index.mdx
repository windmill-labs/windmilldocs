---
description: Manage Windmill instance configuration declaratively with YAML files, Kubernetes CRDs, or the CLI.
---
...
If you deploy Windmill on Kubernetes, using the operator is the recommended way to manage instance configuration. It watches `WindmillInstance` Custom Resources and continuously reconciles the database to match the declared state, re-syncing every 5 minutes to detect and correct configuration drift. Enterprise Edition only.
...
<Tabs>
<TabItem value="helm" label="Via Helm values">
...
<TabItem value="manual" label="Manual CR">
