...
- Choose the **delivery type**:
  ...
  - **Push**: Windmill sets the subscription as a **Push** subscription.
    - For **push delivery**, Windmill provides a **randomly generated URL** following the format:\
      `{base_endpoint}/api/gcp/w/{workspace_id}/{random_string}`.
    - When creating or updating a **push** subscription, Windmill allows you to configure:
      ...
      - An optional **Audience** value (default: the push URL itself).
...
- If the subscription is of **push delivery** type:
  - The subscription's endpoint URL must match the format:\
    `{base_endpoint}/api/gcp/w/{workspace_id}/*`
  - The **path segment** (the portion after `{workspace_id}/`) must be **unique across your workspace**.

  **Example:**

  - Valid:
    - `{base_endpoint}/api/gcp/w/myworkspace/topic-abc`
    - `{base_endpoint}/api/gcp/w/myworkspace/topic-xyz`
  - Invalid:
    - Two subscriptions both using `{base_endpoint}/api/gcp/w/myworkspace/topic-abc`
...
### Using a preprocessor for structured data
...
- **Push delivery failures**: Ensure the push endpoint URL matches the required format:\
`{base_endpoint}/api/gcp/w/{workspace_id}/*`.
