...
### NeonDB
If your Postgres database is hosted on **NeonDB**, special considerations are required when configuring your Postgres resource in Windmill. By default, NeonDB uses **pooled connections** with `pgbouncer`, which are not compatible with Windmill's triggers due to restrictions on the `replica` parameter.

#### Why pooled connections fail by default
NeonDB’s pooled connections are managed using `pgbouncer`. By default, `pgbouncer` allows only a specific subset of startup parameters, and **logical replication**, which Windmill uses to create triggers, requires the `replica` parameter, which is not allowed by default.

If you attempt to use NeonDB's `-pooler` host without modifying the `pgbouncer` configuration, Windmill's triggers will fail to connect because `pgbouncer` will reject the `replica` parameter.

---

#### Avoiding Common Pitfalls: Configuring NeonDB with Windmill
...
Avoid using the `-pooler` host provided by NeonDB.
...
2. Enter the remaining parameters as provided by NeonDB:
...
This configuration works without requiring changes to NeonDB's `pgbouncer` settings.

##### **Good configuration**
For example, if NeonDB provides the following details:
...
- **`port`**: `5432`  
- **`ssl_mode`**: `require`  
- **`root_certificate_pem`**: Provide if required by NeonDB.
...
To use NeonDB’s `-pooler` host with Windmill's Postgres triggers, you must update the `pgbouncer` configuration on your NeonDB instance to allow the `replica` parameter. 

1. **Locate the `pgbouncer` configuration file**:  
   Check NeonDB’s documentation for guidance on accessing and modifying the `pgbouncer` configuration.
...
### Other providers
As other database hosting services are integrated or used, additional guidelines will be provided in this section. Always review your database hoster's documentation for compatibility with long-lived connections required by Windmill's Postgres triggers.
