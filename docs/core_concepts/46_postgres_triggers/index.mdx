...
With `USING INDEX index_name`, the replica identity tracks the columns of a unique index, and this index must cover all the columns that are **NOT NULL**.

- **Correct configuration**: 
    - The trigger must track all the columns that are included in the unique index.
    - If the `bakery` table has a unique index (e.g., `idx_bakery_name_address`) covering `name` and `address`, the trigger should track these two columns.

- **Incorrect configuration**:
    - If the columns tracked by the trigger do not match those in the unique index, `UPDATE` and `DELETE` transactions will fail.
    - To fix this:
      - Ensure the trigger tracks the columns in the `USING INDEX` replica identity or create an appropriate unique index.
...
- For `USING INDEX`, make sure the tracked columns align with the unique index.
