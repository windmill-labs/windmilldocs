...
Windmill can connect to a [PostgreSQL](https://www.postgresql.org/) database and trigger runnables (scripts, flows) in response to database transactions (INSERT, UPDATE, DELETE) on specified tables, schemas, or the entire database.  
Listening is done using PostgreSQL's **logical replication streaming protocol**, ensuring efficient and low-latency triggering.  

---

## Table of Contents
1. [How to Use](#how-to-use)
   - [Create a Postgres Trigger](#create-a-postgres-trigger)
   - [Set Up a PostgreSQL Resource](#set-up-a-postgresql-resource)
   - [Define What to Track](#define-what-to-track)
     - [All Tables](#all-tables)
     - [Specific Schemas](#specific-schemas)
     - [Specific Tables](#specific-tables)
   - [Limitations and Examples](#limitations-and-examples)
     - [Valid Configuration](#valid-configuration)
     - [Invalid Configuration](#invalid-configuration)
   - [Additional Options](#additional-options)
     - [Filtering Rows with WHERE Condition](#filtering-rows-with-where-condition)
     - [Selecting Specific Columns](#selecting-specific-columns)
2. [Advanced](#advanced)
   - [Managing PostgreSQL Publications](#managing-postgresql-publications)
   - [Managing PostgreSQL Replication Slots](#managing-postgresql-replication-slots)
3. [Creating a Script from Tracked Tables](#creating-a-script-from-tracked-tables)
   - [Prerequisites](#prerequisites)
   - [How to Use](#how-to-use-1)
   - [Key Notes](#key-notes)
   - [Example Schema and Table](#example-schema-and-table)
...
### Set Up a PostgreSQL Resource
You need to either:
- **Create a new PostgreSQL resource** by providing:
  - Hostname, port, database name, username, and password.
  - Advanced options such as SSL settings if needed.
- **Reuse an existing PostgreSQL resource**.

### Define What to Track
Once the PostgreSQL resource is configured, you can choose what to track.
...
- If your trigger is set to track `UPDATE` and/or `DELETE` transactions, the `WHERE` clause **can only reference columns that are part of the table’s replica identity**.  
  See the [REPLICA IDENTITY documentation](https://www.postgresql.org/docs/current/sql-altertable.html#SQL-ALTERTABLE-REPLICA-IDENTITY) for more details.
- If your trigger tracks only `INSERT` transactions, the `WHERE` clause can reference **any column**.

For more details, refer to the [PostgreSQL WHERE clause documentation](https://www.postgresql.org/docs/current/logical-replication-row-filter.html#LOGICAL-REPLICATION-ROW-FILTER-RESTRICTIONS).
...
### Managing PostgreSQL Publications
...
By providing this flexibility, you can directly interact with your PostgreSQL database to better control the publications used by your triggers.

For more information on **PostgreSQL publications**, refer to the [PostgreSQL documentation on publications](https://www.postgresql.org/docs/current/logical-replication-publication.html).

---

### Managing PostgreSQL Replication Slots
...
For more information on **PostgreSQL replication slots**, refer to the [PostgreSQL documentation on replication slots](https://www.postgresql.org/docs/current/logical-replication-slot.html).

---

## 5. Creating a Script from Tracked Tables
...
- **PostgreSQL Resource**: A PostgreSQL resource must be configured in your environment to enable this feature.
...
1. **Set up PostgreSQL Resource**: Ensure that a PostgreSQL resource is configured in the resources page.
...
- **row**: Contains the data of the row involved in the transaction. The data type of each field in `row` depends on the column's data type in the PostgreSQL table.
...
![Generated script template example](./generated_script_template_example.png 'Generated script template for tracked tables')

This template will process the transaction types (`insert`, `update`, or `delete`), handle the schema and table names, and include the row data for processing.