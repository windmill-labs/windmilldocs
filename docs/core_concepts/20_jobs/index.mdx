...
You can run a job and get an SSE stream of the result using the [SSE stream webhooks](../4_webhooks/index.mdx#sse-stream-webhooks) or an [HTTP route](../39_http_routing/index.mdx) in Sync SSE mode.
