...
{/* Placeholder image: screenshot of the Windmill run page showing a WAC workflow in "suspended" state. The run detail should show the job status as "Suspended", the timeline with some completed child tasks and a pending suspension point (e.g., waiting for approval or sleeping). The left panel should show the parent job metadata including the workflow script path. This illustrates how the worker slot is freed while the workflow waits. */}

{/* Placeholder image: screenshot of the Windmill script editor with a WAC v2 TypeScript script open. The editor should show the code with workflow(), task(), and step() calls. The right panel should show the "Test" tab with input fields and a "Run" button. If modules are present, show the module tabs at the top of the editor (e.g., "script.ts", "extract.ts"). */}
...
export const main = workflow(async (endpoints: string[]) => {
  ...
  const transformed = await parallel(
    allRecords,
    (record) => task(transformRecord)(record),
    { concurrency: 20 },
  );
...
```typescript
import {
  task, step, workflow, sleep, waitForApproval, getResumeUrls, taskScript
} from 'windmill-client';
...
const result = await waitForApproval({
  ...
  form: {         // optional JSON Schema for approval form
    type: 'object',
    properties: {
      reason: { type: 'string' }
    }
