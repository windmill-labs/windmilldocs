...
{/* Placeholder image: two screenshots side by side. Left: the job detail page showing the workflow in "suspended" / waiting-for-approval state with the approve/reject buttons (and form fields if using a form). Right: the timeline view after approval showing the completed workflow with tasks before and after the approval point. */}
