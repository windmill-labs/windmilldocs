...
| `form` | `object` / `dict` | `undefined` / `None` | JSON Schema describing form fields shown on the approval page |
| `selfApproval` / `self_approval` | `boolean` / `bool` | `true` / `True` | Whether the user who triggered the workflow can approve it. Set to `false` to require a different user. See [disable self-approval](../../flows/11_flow_approval.mdx#disable-self-approval) |
...
// approval.value contains the submitted form data, e.g. { reason: "LGTM", environment: "production" }
```
...
| `form` | `object` | `undefined` | JSON Schema for an approval form |
| `selfApproval` | `boolean` | `true` | Allow the triggering user to approve. Set to `false` to require a different approver |
...
// result.value    — submitted form data (if form was provided)
```
...
| `form` | `dict \| None` | `None` | JSON Schema dict for an approval form |
| `self_approval` | `bool` | `True` | Allow the triggering user to approve. Set to `False` to require a different approver |
