...
For large logs storage (and display) and cache for distributed Python jobs, you can [connect your instance to a bucket](../20_jobs/index.mdx#large-logs-management-with-s3). This feature is at the Instance-level, and has no overlap with the Workspace object storage.
...
This feature is available on [Enterprise Edition](/pricing) and is configurable from the [instance settings](../../advanced/18_instance_settings/index.mdx#s3azure-for-pythongo-cache--large-logs).
