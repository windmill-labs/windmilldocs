...
Connect your Windmill workspace to your S3 bucket, Azure Blob storage, or Google Cloud Storage to enable users to read and write from object storage without having to have access to the credentials.
...
Once you've created an [S3](../../integrations/s3.mdx), [Azure Blob](../../integrations/microsoft-azure-blob.md), or [Google Cloud Storage](../../integrations/google-cloud-storage.mdx) resource in Windmill, go to the workspace settings > S3 Storage. Select the resource and click Save.
...
Google Cloud Storage (GCS) is fully compatible with Windmill's S3 integration through Google's S3-compatible interoperability API. This means you can use all Windmill object storage features with GCS buckets.

To use Google Cloud Storage with Windmill:

1. **Create HMAC keys** for your GCS service account in the Google Cloud Console
2. **Configure the S3 resource** using:
   - **Endpoint**: `storage.googleapis.com`
   - **Access Key/Secret Key**: Your HMAC credentials
   - **Region**: Your GCS bucket region (e.g., `us-central1`)
3. **Use the same S3 functions** (`loadS3File`, `writeS3File`, etc.) in your scripts

All workspace and instance object storage features work seamlessly with GCS, including:
- File upload/download through Windmill UI
...
<div className="grid grid-cols-2 gap-6 mb-4">
	<DocCard
		...
		description="Complete setup guide for using Google Cloud Storage with Windmill's S3-compatible object storage features."
