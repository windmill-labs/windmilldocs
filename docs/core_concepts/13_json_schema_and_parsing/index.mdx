...
In flows, dynamic select is only available for flow input steps. You must define one function for each flow input field that is of dynamic select type. The function name must exactly match the name of the flow input field. No export type is required - just define the function inline within the flow editor:
...
The select options recompute dynamically based on other input arguments. You can also use `text` as an argument in the function to access the current text input value for filtering.

You can implement custom filtering and sorting logic within your function. In the examples above, when `x` equals "bar", the options are filtered to show only one option.
...
<!-- TODO: Add video showing dynamic select usage in flows -->

<video
	className="border-2 rounded-xl object-cover w-full h-full dark:border-gray-800"
	controls
	src="/videos/dynamic_select_flows.mp4"
/>
