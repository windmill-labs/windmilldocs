...
| [DynMultiselect_foo](#dynamic-multiselect) | `dynmultiselect-<name>` |
...
Dynamic select is a helper function that allows you to create a select field with dynamic options that recompute based on other input values. It's available in both [scripts](../../script_editor/index.mdx) and [flows](../../flows/1_flow_editor.mdx), but with different implementation requirements. For flows, dynamic select and multiselect can also be used for flow steps that are scripts.
...
Dynamic multiselect works exactly like dynamic select but renders a multiselect component that allows selecting multiple values from the dynamic options. The function implementation is identical to dynamic select, but it returns an array of selected values instead of a single value.

### Dynamic multiselect in scripts

For dynamic multiselect in scripts, you must export both the string array type as `DynMultiselect_<name>` and the function as `<name>`. The function implementation is identical to dynamic select, but the type is an array of strings:
...
export async function categories(x: string, y: number, text: string) {
	if (text === '42') {
		return [{ value: '42', label: 'The answer to the universe' }];
	}
	if (x === 'bar') {
...
export async function main(y: number, x: string, categories: DynMultiselect_categories) {
...
```python
DynMultiselect_categories = list[str]

def categories(x: str, y: int, text):
  if text == "42":
    return [{"value": "42", "label": "The answer to the universe"}]
  if x == "bar":
    return [{"value": "barbar", "label": "barbarbar"}]
...
def main(x: str, y: int, categories: DynMultiselect_categories):
	print(categories)  # Array of selected values
	return categories
```
...
### Dynamic multiselect in flows

In flows, dynamic multiselect works the same as dynamic select - it's only available for flow input steps. The function name must exactly match the name of the flow input field. The function implementation is identical to dynamic select, but the input field will render as a multiselect component that returns an array of selected values.

<Tabs className="unique-tabs">
<TabItem value="typescript" label="TypeScript" attributes={{className: "text-xs p-4 !mt-0 !ml-0"}}>

```ts
// Function for flow input field named "categories"
async function categories(x: string, y: number, text: string) {
	if (text === '42') {
		return [{ value: '42', label: 'The answer to the universe' }];
	}
	if (x === 'bar') {
...
// If you have another dynamic multiselect input named "tags"
...
### How dynamic multiselect works

Dynamic multiselect works identically to dynamic select - the options recompute dynamically based on other input arguments. The only difference is that it renders a multiselect component allowing multiple value selection and returns an array of selected values instead of a single value.

You can implement the same custom filtering and sorting logic within your function. When the available options change, any previously selected values that are no longer available will be automatically filtered out to maintain data consistency.
