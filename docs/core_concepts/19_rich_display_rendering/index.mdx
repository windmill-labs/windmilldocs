...
Some formats are recognised automatically [Rich Table Display](#rich-table-display), while others can be forced through your code. By leveraging specific formats, you can display images, files, tables, HTML, JSON, and more.
...
| ----------       | ----------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| [json](#json)             | Render the value as a JSON.                                  | `return { "json": { "a": 1 } }`                                                                                      |
...
The `json` key allows returning the results in a JSON format.

```ts
return { "json": { "a": 1 } }
```

![Rich display JSON](./json.png "Rich display JSON")
...
If the result matches the table format (either `table-col`, `table-row`, or `table-row-object`), it will be displayed as a table even if the data is not nested under the key `table-*`.
...
The `table-row` key allows returning the value as a row-wise table but where each row is an object (optionally the first row can be an array of strings to enforce column order).	
...
If the files returned are CSV or parquet files, Windmill will display a [table preview](#rich-table-display).
