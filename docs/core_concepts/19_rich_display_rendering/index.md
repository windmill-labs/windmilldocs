...
| [s3](#s3)               | Render S3 files as a downloadable file and a bucket explorer, when Windmill is [connected to a S3 storage](../11_persistent_storage/large_data_files.mdx).                                  | `return { "s3": "path_to_file"}`           |
...
| [error](#error)            | Render the value as an error message.                        | `return { "error": { "name": "418", "message": "I'm a teapot" }}`                                                    |
...
As you can see, the columns are not properly ordered. You can force column order with [Table Row Object](#table-row-object).
...
List of columns is not mandatory but it allows [forcing their order](#example).

```ts
return { "table-row-object": [ { "foo": 42, "bar": 38 }, { "foo": 8, "bar": 12 } ] }
```
...
```ts
return { "table-row-object": [ ["foo", "bar" ], { "foo": 42, "bar": 38 }, { "foo": 8, "bar": 12 } ] }
```
...
The `s3` key renders S3 files as a downloadable file and a bucket explorer, when Windmill is [connected to a S3 storage](../11_persistent_storage/large_data_files.mdx).
