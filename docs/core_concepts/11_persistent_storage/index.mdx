...
3. Under the account, go to "Containers" in the left menu bar and create a new one. Azure's containers are roughly the equivalent of an S3 bucket. Note though that secret access key are per account, not per container.
