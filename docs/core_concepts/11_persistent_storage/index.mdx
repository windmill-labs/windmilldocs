...
You can store Windmill-specific elements with [resources](../3_resources_and_types/index.mdx), [variables](../2_variables_and_secrets/index.mdx) etc. To store data, it is recommended to use data tables, or external storage service providers that can be accessed from Windmill.
...
For each computation the worker executing is not the same as the previous computation, so the data would have to be retrieved from another location. Windmill is very convenient to use alongside data storage providers to manipulate big amounts of data.
