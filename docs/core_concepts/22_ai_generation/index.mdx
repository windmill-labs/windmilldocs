...
[Enterprise](/pricing) customers can run all Windmill AI features on any custom AI provider that is OpenAI API-compatible using the `Custom AI` tab.

For Azure OpenAI models, you should use the OpenAI provider and specify the base path of the model in the [instance settings](../../advanced/18_instance_settings/index.mdx#azure-openai-base-path).