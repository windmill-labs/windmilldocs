...
## Interactive SSH REPL for Worker Debugging
...
### Key capabilities:
- Launch a REPL terminal connected to the host of a specific worker.
- Run interactive bash commands with support for:
  - Changing directories via `cd`.
  - Multi-line and compound commands.
  - Auto-output formatting for results.
- View and cancel pendings jobs directly from the terminal session.
- Easily paste and run bash snippets, or open from existing workspace/hub scripts.

> The REPL uses the same job execution backend as regular scripts, meaning commands are executed via short-lived bash jobs, offering a secure and reproducible environment.
...
**Note:**  
- Use `cd` to navigate directories — the shell remembers your working directory across commands.  
