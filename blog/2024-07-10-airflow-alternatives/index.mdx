...
![Compara table](./compare_airflow.png "Compara table")
...
With [Temporal](https://github.com/temporalio/temporal), you move into the realm of developers, moving away from the domain strictly reserved for data engineers. Temporal is a runtime that aims to ensure the durable execution of your application code, notably through fault tolerance (Temporal promises to "reduce failure by 10-100x").
...
Windmill is an [open-source](https://github.com/windmill-labs/windmill) workflow engine and developer platform designed to build internal tools, including endpoints, workflows, and UIs. It supports coding in multiple languages such as TypeScript, Python, Go, Bash, SQL, or any Docker image, alongside low-code builders.
...
On the other hand, the support of [Python](/docs/getting_started/scripts_quickstart/python) as a primary language and the integration of your workspace with [object storage](/docs/core_concepts/persistent_storage/large_data_files) (in particular, S3) make Windmill an excellent fit for data engineers, particularly for building [data pipelines](/docs/core_concepts/data_pipelines).

Windmill has three editors, all compatible, each independently functioning:
...
3. The [App Editor](/docs/apps/app_editor) is a tool for creating customized, user-friendly interfaces using a drag-and-drop editor, allowing you to build data-centric dashboards.
...
- **Comprehensiveness**: Windmill supports multiple languages and serves as an all-in-one platform for iteration, triggers (schedules, webhooks etc.), and deployments.
- **Developer Experience**: Windmill offers features like writing scripts locally, testing them step by step, or running full flows in the web UI.
...
[Mage](https://github.com/mage-ai/mage-ai) is a newcomer that recently emerged in [2020](https://m.mage.ai/mage-vs-airbyte-93fba4dc09cb), founded by former engineers from Airbnb. Mage shares the same promise as Prefect or Dagster: to be a modern solution aiming to replace Airflow (they mention Airflow 5 times on their landing page alone).
...
At last, you can take a look at [Kestra](https://github.com/kestra-io/kestra), the youngest of these alternatives to Airflow. Created in 2021 (by Emmanuel Darras, the founder of Ankama), Kestra is an event-driven orchestration platform to manage business-critical workflows declaratively in code.
Kestra has a declarative YAML syntax and all operations are driven through its API.

For more details on the comparison with Airflow, check out this [article](https://kestra.io/vs/airflow) written by Kestra.
...
To find the right tool, it's important to identify your needs: what skills are available internally (incl. mastered languages), [performance at scale](/docs/misc/benchmarks/competitors), whether you want to do just data pipelines or also build a stack of internal tools around, whether you are ready to support onboarding on a complex product, and whether you might need support (and a company accountable to you).

The advantage is that all these alternatives are open source and easily self-hostable, to get your own idea for free, and without receiving emails from overly wilful salespeople.

Finally, your internal infrastructure constraints will be an important criterion, for example, Prefect, Dagster, and Kestra might be used for the same purposes, but they have different infrastructures. I suggest you take a look at the documentation for each software to find out more.

Please note that one of these alternatives (Windmill) has presented this article to you. Please contact us if you find any inaccuracies or would like to contribute. Feel free to make a PR on [windmilldocs](https://github.com/windmill-labs/windmilldocs) (open source to the end).