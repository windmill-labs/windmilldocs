...
It can suffer from 2 performances penalties, cold start to spawn the runtime and lack of JIT
...
bash has no cold start, and go scripts are compiled AOT and then cached locally to avoid any cold starts. The only remaining
```
