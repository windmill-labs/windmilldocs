...
- **Credential isolation**: API keys live in the encrypted [resource store](/docs/core_concepts/resources_and_types), not in the agent's environment. Windmill supports [multiple secret storage backends](/docs/core_concepts/variables_and_secrets): its own encrypted store, HashiCorp Vault, Azure Key Vault, and AWS Secrets Manager. Either way, the agent never sees them.
