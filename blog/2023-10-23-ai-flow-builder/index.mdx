...
title: Launchweek day X - Flow Builder Copilot
authors: [hugocasa]
tags:
  [
    'Windmill AI',
...
This feature allows you to easily create a workflow by simply describing it in plain English or by picking scripts from the hub.
The copilot will take care of generating each step and link them together automatically.
...
![Windmill DAG](./media/windmill-dag.png)

Workflows are series of steps connected together through a [DAG](https://en.wikipedia.org/wiki/Directed_acyclic_graph), each step being a script to be executed.
Through the structure and connection of the steps, we define in what order the steps should be executed.

In addition, a workflow is composed of a separate dataflow graph which defines what data is transferred between the steps, as well as of input transforms which prepare the inputs for each step.
Windmill flows support many script languages and can contain for-loops, branches (parralellizable), and much more. Learn more about flows [here](/docs/getting_started/flows_quickstart).

## Why an AI flow builder?

Workflows can be quite complex and tedious to create, especially connecting steps together, therefore we decided to build an AI copilot to help you create workflows faster and easier.

However, asking GPT-4 to generate a complete workflow is too hard of a problem: it is too difficult to control and would most of the time generate something that you don't want.
A good compromise is to first have the user give a structured prompt that mirrors the topologic layout of the DAG:

![Structured prompts](./media/structured-prompts.png)

Then, the AI assistant can step in to build the workflow, one step at a time.
It generates the code based on the given prompt and workflow context, and more importantly, it establishes the connections between the steps.
At the end of each step, the user can review and edit each step as needed.
And If you're not happy at all with the generated step, you can always edit the instructions and regenerate the step or even the whole flow if needed:
...
Therefore you get the best of both worlds: most of the boilerplate, tedious aspect of workflows are accelerated by AI, but you are still in full control of the creation process.

Moreover, generating script from scratch is sometimes not necessary as there could a script in the [hub](https://hub.windmill.dev/) that already does what you want, so we give you the option:

![Flow builder step options](./media/flow-builder-step-options.png)

And if you want even more control, you can ask to generate only one step wherever you want in the flow:
...
We thought that it would be too slow or too expensive, but after some testing and optimization, we realized that it was actually quite fast, cheap (thanks to GPT-3.5) and powerful.
