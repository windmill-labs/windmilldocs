...
The order of the steps execution is defined by the DAG's structure.
A worflow also consists of a separate dataflow graph which defines what data is transferred between the steps, and of input transforms which prepare the inputs for each step.
...
Workflows can be quite complex and tedious to create, especially data connections, therefore we decided to build an AI copilot to help you create workflows faster and easier.

However, asking GPT-4 to generate a complete workflow is too hard of a problem: it is too difficult to control and would most of the time generate something that you do not want.
A good compromise is to first have users give a structured prompt that mirrors the topologic layout of the DAG:
...
At the end of each step, you can review and edit each step as needed.
And if you're not happy with the generated step or changed your mind, you can always edit the instructions and regenerate the step or even the whole flow if needed:
...
Moreover, generating script from scratch is sometimes not necessary as there could be a script in the [hub](https://hub.windmill.dev/) that already does what you want, so we give you the option:
