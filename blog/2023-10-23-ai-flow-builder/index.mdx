...
You can see below an example of a simple workflow with a for-loop:
...
When selecting a hub script instead of generating a step from scratch, we do not know what input should receive the output of the previous step, therefore we ask GPT-4 to fill in the step's inputs based on the previous step's code and the selected script's inputs.
