...
The particularity of data pipeline flows VS any other kind of automation flows is that they are run computation on large datasets and the result of such computation is itself a (potentially large) dataset that needs to be stored.
...
- Running a `SELECT COUNT(*), SUM(column_1), AVG(column_2) FROM my_table GROUP_BY key` with _300M_ entries in `my_table` requires less than _11Gb_ of memory using DuckDB
...
And for the storage, we've made some improvements recently that allows you to connect your Windmill workspace to an S3 bucket and interact with it almost seamlessly.

The very large majority of ETL can be processed step-wise on single nodes and Windmill provides (one of) the best model for orchestrating non-sharded compute. Using this model, your ETL will see a massive performance improvement, your infrastructure
...
From now on, Windmill will be connected to this bucket and you'll have an easy access to it from the code editor and the job run details. If a script takes an input an `s3object` for example, you will see in the input form on the right a button suggesting you to pick a file directly from the bucket.
...
Again, you always have the possibility to use your S3 client library of your choice to do this yourself.
...
And more to come! With both Windmill providing the boilerplate code, and Polars and DuckDB handling reading and writing from/to S3 natively, you can interact with S3 files very naturally and your Windmill scripts becomes concise and focused on what really matter: processing the data.

In the end, a canonical pipeline step in Windmill will look something like:
