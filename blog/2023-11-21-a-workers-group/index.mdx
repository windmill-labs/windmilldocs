...
In a self-hosted instance, worker groups are created from your `docker-compose.yml` by simply passing the worker group as the env variable `WORKER_GROUP=worker_group_name`.
It is as simple as that and means that workers can be scaled horizontally.

The configuration of worker groups is done from the Windmill UI' [Workers page](/docs/core_concepts/worker_groups), pick "New worker group config" and just write the name of your worker group.
