...
We also took the time to explain in details how we were able to achieve this: link
...
What is convenient about this design is that **every state transaction is just one transactionned postgresql statement**. Postgresql is ACID, and we can leverage those properties to the full extent. A lot of what is hard and slow in a workflow engine is to reach eventual consistency.
We skip the hard part by:

- using Postgres which implements MVCC and row-locks
...
However, we implemented recently [dedicated worker for scripts](local link), which spawn a a python process once at the start of the worker and then execute the script logic in a while loop, taking the job inputs in stdin and returning the output to sdout.
