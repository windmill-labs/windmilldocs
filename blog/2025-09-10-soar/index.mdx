...
A few major orgs (10k employees with 50+ security teams) have migrated from specialized SOAR product leaders like Palo Alto Networks, Tines etc. to Windmill thanks to the freedom, performance and flexibility they can find in a more generic and more power-user oriented tool like Windmill. In this blog post, we will see how Windmill is leveraged efficiently by those orgs for the SOAR use-case.
...
This is where Windmill's AI Agent flow step comes into play. An AI flow step can use tools (e.g calling VirusTotal, a Threat Intelligence API, etc.) as well as outputs from other steps to generate a structured response based on an output schema provided.
